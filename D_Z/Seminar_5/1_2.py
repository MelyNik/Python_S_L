...
# Самостоятельно не справился, только разбрав уже готовое.
...
def Encode(text='text.txt' , final_text='text_f.txt'):
    if path.exists(text) and not path.exists(final_text): # Проверяем наличие пути только файла text.
        # path.exists - йункция которая проверяет наличие пути у файла или его отсутствие.
